...
def description(func_name='name'):
	if str(func_name) == 'clean':
		...
		details = ['Y: Y-values of data', 'NSIG: The number of standard deviations away from the MEAN a Y-value in Y must be to qualitfy as a peak', 
		...
		details = ['FILENAME: Name of raw data file', 'FILEWRITE: User-designated name of reduced data fi    le' , 'HDR: Header index of raw image in FILENAME', 'NSIG: The number of standard deviations away from the MEAN a Y-value in Y must be to qualitfy as a peak', 'WINDOW: Size of the smoothing window used in scipy.signal.medfilt()', 'WINDOW2: Second, larger smoothing window for scipy.signal.medfilt()', 'WRITE: True: Save image to FILEWRITE\n False: Do not save to FILEWRITE']
...
def clean(FILENAME, FILEWRITE, FLIP, CUT, CEILING, SCAN, WRITE=True, HDR=0, HIRES = False):
    ...
    dark = np.arange(CUT,nrows*ncols,ncols) #dark is the last column of pixels at which this cutoff occurs and
                                         #only darker areas that are not part of the orders remain. For example,
                                         #if there are 50 columns of darkness after the orders cutoff,
    ...
    if WRITE: #Write to file
        ...
        hdulist.writeto(str(FILEWRITE), overwrite = True)
    return image
    #Returns the trimmed image

# peaks() defines the orders in coordination with trace()
def peaks(Y, NSIG, MEAN = -1, STDEV = -1):
    right, left = Y - np.roll(Y,1), Y - np.roll(Y,-1)
        #Shift the elements in Y left and right and subtract this from
        #the original to check where these values are > 0
    pk = np.where(np.logical_and(right > 0, left > 0))[0]
    if NSIG <= 0.0:                     
        print 'Setting NSIG = 1.0 in peaks()'
        NSIG = 1.0
    if NSIG > 0.0:
        if type(Y) != type(np.array(0)): #Verify lists and arrays are not interacting so Y can be
            Y = np.array(Y)              #indexed with pk
        yp = Y[pk]
    ...
    #Returns the indices of Y at which NSIG-constrained maxima are
...
def trace(IMAGE, XSTART, YSTART, XSTEP, YRANGE, NSIG, FILEWRITE, SEP, WRITE=False, ODR=False, PROFIT = True):
    #############################################################################################################
