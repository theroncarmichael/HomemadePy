...
def clean(FILENAME, FILEWRITE, FLIP, CUT, CEILING, SCAN, WRITE=True, HDR=0, HIRES = False):
    ...
    image_file.close()
    image = image[[~np.any(np.isnan(image),axis = 1)]] #Remove NaN values and collapse from 2D to 1D
    ...
    print 'Subtracting the bias (median of the overscan region)...'
    #image -= np.median(bias)
    image[ image <= 0 ] = 1.0
...
#	image = np.delete(image, np.s_[0:10:1], axis = 1) # axis = 1 deletes columns (0 to 10 here)
